...
//Test for Set 1 challenge 5
func TestRepeatingKeyXor(t *testing.T) {
    plaintext := []byte("Burning 'em, if you ain't quick and nimble\nI go crazy when I hear a cymbal")
    key := []byte("ICE")
    ciphertext := encoding.BytesToHex(RepeatingKeyXor(key, plaintext))
    ...
    if ciphertext != expected_ciphertext {
        t.Errorf("RepeatingKeyXor: Expected output does not match actual output.")
    }
}
